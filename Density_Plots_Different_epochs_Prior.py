...
def extract_epoch(filename):
    # Use regular expression to find the pattern "agent_XXep_SAScore.csv"
    match = re.search(r'prior_(\d+)ep', filename)
    if match:
...
def main(file_paths):
    ...
    plt.xlabel('SAScore')
    plt.ylabel('Density')
    plt.title('Density Plot - SAScores - Different Epochs in Prior ')
    plt.legend()
