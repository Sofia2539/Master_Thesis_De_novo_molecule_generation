...
components / Different Matching Substructure/ Custom Alert/ Weights"""
...
df1_modified= pd.read_csv("scores_smiles_filtered_final_test_prior_20ep_agent_20ep_small_modifications_weights_components.csv")

# Read dataset with generated molecules focused on PIM1 with big memory and sample size
df1= pd.read_csv("scores_smiles_filtered_final_test_prior_20ep_agent_20ep.csv")


# Add a column in order to indicate the source dataset
df1_modified["Modifications"] = 'Different Custom_Alert & Matching_Substructure - increased Weights in Components'

df1["Modifications"] = 'None'

# Concatenate the datasets
combined_df = pd.concat([df1_modified, df1], ignore_index=True)

# Process the SMILES strings
invalid_smiles = []
for idx, row in combined_df.iterrows():
    smiles = row["SMILES"]
    dataset = row["Modifications"]
    try:
        mol = Chem.MolFromSmiles(smiles)
        if mol is None:
            invalid_smiles.append((smiles, dataset))
    except Exception as e:
        print(f"Error processing SMILES: {smiles} (Modifications: {dataset}) - {e}")

# Invalid SMILES & corresponding datasets
for smiles, dataset in invalid_smiles:
    print(f"Invalid SMILES: {smiles} (Modifications: {dataset})")
...
crds_df['Modifications'] = remaining_df["Modifications"]


# Set the size of the plot
sns.set(rc={'figure.figsize': (16, 10)}) 
sns.set_style('whitegrid')
ax = sns.scatterplot(data=crds_df, x="PC_1", y="PC_2", hue="Modifications", palette={'Different Custom_Alert & Matching_Substructure - increased Weights in Components': "red",'None': "green"})
# Add a bold title to the plot
plt.title('PCA - use case - Small Modifications on the Components', fontweight='bold')
plt.show()
...
tsne_df['Modifications'] = remaining_df["Modifications"]
...
ax = sns.scatterplot(data=tsne_df, x="X", y="Y", hue="Modifications", palette={'Different Custom_Alert & Matching_Substructure - increased Weights in Components': "red",'None': "green"})
plt.title('t-SNE - use case - Small Modifications on the Components', fontweight='bold')
plt.show()
